...
tc0_enable(2,2); //2,2
tc1_enable(4,2); //4,2
hx = hx711_enable(&dev()->gpiof->ddr.var, &dev()->gpiof->pin.var, &dev()->gpiof->port.var, 6, 7); //6,7
//intx = INTERRUPTenable();
lcd0_enable(&dev()->gpioa->ddr.var,&dev()->gpioa->pin.var,&dev()->gpioa->port.var);
