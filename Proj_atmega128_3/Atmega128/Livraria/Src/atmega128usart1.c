...
void usart1_enable( uint32_t baud, unsigned int FDbits, unsigned int Stopbits, unsigned int Parity )
{
	...
	dev()->usart1->ubrr1h.var = writeHLbyte(ubrr).par.h.var;
	dev()->usart1->ubrr1l.var = writeHLbyte(ubrr).par.l.var;
	// Enable USART receiver and transmitter and receive complete interrupt
	dev()->usart1->ucsr1b.var = (1 << RXCIE1) | (1 << RXEN1)|(1 << TXEN1);
		...
		switch(FDbits){
			case 9:
				dev()->usart1->ucsr1b.var |= (1 << UCSZ12);
				dev()->usart1->ucsr1c.var |= (3 << UCSZ10);
			break;
			case 8:
				dev()->usart1->ucsr1b.var &= ~(1 << UCSZ12);
				dev()->usart1->ucsr1c.var |= (3 << UCSZ10);
			...
			case 7:
				dev()->usart1->ucsr1b.var &= ~(1 << UCSZ12);
				dev()->usart1->ucsr1c.var |= (1 << UCSZ11);
				dev()->usart1->ucsr1c.var &= ~(1 << UCSZ10);
			break;
			case 6:
				dev()->usart1->ucsr1b.var &= ~(1 << UCSZ12);
				dev()->usart1->ucsr1c.var &= ~(1 << UCSZ11);
		...
		switch(Parity){
			case 0: // NONE
				dev()->usart1->ucsr1c.var &= ~(3 << UPM10);
			break;
			case 2: // EVEN
				dev()->usart1->ucsr1c.var |= (1 << UPM11);
				dev()->usart1->ucsr1c.var &= ~(1 << UPM10);
			break;
			case 3: // ODD
				dev()->usart1->ucsr1c.var |= (3 << UPM10);
			break;
...
{
	uint16_t timeout;
	dev()->usart1->ucsr1b.var |= 1 << UDRIE1;
	dev()->usart1->udr1.var = data;
...
static void usart1_callback_rx(void)
{
	...
	bit9 = dev()->usart1->ucsr1b.var;
	bit9 = 0x01 & (bit9 >> 1);
	
	if(usr){ UART1_LastRxError = usr; }
	
	UART1_Rx = dev()->usart1->udr1.var;
