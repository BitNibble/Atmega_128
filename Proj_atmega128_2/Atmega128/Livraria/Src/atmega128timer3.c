...
// for more information read data sheet.
{
	...
	dev()->tc3->ocr3a = writeHLbyte(~0);
	dev()->tc3->ocr3b = writeHLbyte(~0);
	dev()->tc3->ocr3c = writeHLbyte(~0);
}
...
void TIMER_COUNTER3_wavegenmode(unsigned char wavegenmode)
{
	dev()->tc3->tccr3a.var &= ~((1 << WGM31) | (1 << WGM30));
	dev()->tc3->tccr3b.var &= ~((1 << WGM33) | (1 << WGM32));
...
void TIMER_COUNTER3_interrupt(unsigned char interrupt)
{
	dev()->tc3->etimsk.var &= ~((1 << TICIE3) | (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C));
	switch(interrupt){ // ICP3  -->  PE7
		case 0:
		break;
		case 1:
		dev()->tc3->etimsk.var |= (1 << TOIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 2:
		dev()->tc3->etimsk.var |= (1 << OCIE3A);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 3:
		dev()->tc3->etimsk.var |= (1 << OCIE3B);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 4:
		dev()->tc3->etimsk.var |= (1 << OCIE3C);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 5:
		dev()->tc3->etimsk.var |= (1 << TICIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 6:
		dev()->tc3->etimsk.var |= (1 << OCIE3A) | (1 << TOIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 7:
		dev()->tc3->etimsk.var |= (1 << OCIE3B) | (1 << TOIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 8:
		dev()->tc3->etimsk.var |= (1 << TOIE3) | (1 << OCIE3C);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 9:
		dev()->tc3->etimsk.var |= (1 << TICIE3) | (1 << TOIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 10:
		dev()->tc3->etimsk.var |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 11:
		dev()->tc3->etimsk.var |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << TOIE3) | (1 << OCIE3C);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		case 12:
		dev()->tc3->etimsk.var |= (1 << OCIE3A) | (1 << OCIE3B) | (1 << OCIE3C);
		dev()->cpu->sreg.var |= 1 << 7;
		break;
		default:
		break;
